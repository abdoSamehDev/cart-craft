...
const AdminLoginPage: React.FC = (): JSX.Element => {
  ...
  const [loginState, fetchLogin] = useFetch<{ token: string }>('https://dummyjson.com/auth/login', {
    method: 'POST',
  ...
  return (
      <div className="flex items-center justify-center h-screen bg-gray-100">
        <Card className="max-w-lg w-full"> {/* Increased width */}
          <form onSubmit={handleLogin} className="flex flex-col gap-4">
            ...
            {loginState.error && <p className="text-red-500">{loginState.error.message}</p>}
          </form>
        </Card>
      </div>
  );
